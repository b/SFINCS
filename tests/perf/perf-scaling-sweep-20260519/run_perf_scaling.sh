#!/bin/bash
# SOR-1017 — scaling sweep across simulation length, configuration,
# and grid resolution.
#
...
SOR-1017 scaling-sweep capture environment
==========================================
host CPU         : $HOST_CPU
...
exec > >(tee -a "$LOG") 2>&1
echo "=== SOR-1017 scaling sweep — start $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
echo "cases  : ${CASES[*]}"
...
echo
echo "=== SOR-1017 scaling sweep — complete $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
