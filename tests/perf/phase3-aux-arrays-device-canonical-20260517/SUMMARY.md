...
The remaining inner-loop steady-state per-step D2H is dominated by `cont_subgrid_copyout` (208.2 kB/step — the feature-gated `zs0` / `storage_volume` bridge-OUTs from `compute_water_levels_subgrid` for the host-side wiggle-suppression smoothing and storage-volume balance, both enabled in this case). Those bridge-OUTs are NOT auxiliary-array traffic Phase 3 targeted — they serve the surviving CPU paths and are out of Phase 3's scope; their elimination is a later-phase concern when those paths are ported to the device. The auxiliary-array NVTX ranges Phase 3 DID target — `bnd_fluxes_copyout`, `cont_regular_copyout`, the auxiliary slots in `cont_subgrid_copyout`, `mom_fluxes_copyout` (kfuv), and the auxiliary slots in `cont_*_copyin` — all measure at-or-near-zero per step (see table), satisfying AC #8's "auxiliary NVTX ranges are at or near zero bytes per step".
The other remaining per-step transfers are the sliced boundary H2D `bnd_conditions_copyin` (2.1 kB/step, kcs==2-sized — the AC #1 sliced-rather-than-bulk minimum) and the small residual `mom_fluxes_copyout` (the `min_dt` int32 scalar).
Cadenced bursts paid outside the per-step path: the SnapWave coupling refresh (paid every `dtwave = 1800 s` in this case, i.e. ~1 in every ~30 steps at this case's `dt`) and the output gather (paid every `dtmapout = 86400 s`, i.e. exactly once over this 24-h simulation — outside the steady-state NSight window) are the dominant TOTAL-PCIe contributors when integrated over the full run, but they do not contribute to the per-step bytes shown in the table because the NSight window deliberately captures the 46 s steady-state interior between them. Their per-burst sizes are recorded in `cuda_gpu_mem_size_sum.csv` and aggregated into the `(unattributed)` row above (1.5 kB/step H2D + 3.6 kB/step D2H amortized over the 20,213-step window).
