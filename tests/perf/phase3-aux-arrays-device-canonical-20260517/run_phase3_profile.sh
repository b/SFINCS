...
# capture (zsb / zsb0 / uvmean / kfuv / zsm / maxzsm / zsmax / z_volume /
# zsderv).
#
# Identical methodology to tests/perf/phase2-q-uv-device-canonical-20260517/
# run_phase2_profile.sh (same case, same gpu_n2 config, same CUDA-aware
# HPC-X 2.24 container, same 60 s steady-state NSight window after a 20 s
# startup-skip delay) so the per-NVTX-range H<->D byte attribution is
# directly comparable to the committed Phase 0 / Phase 1 / Phase 2
# captures. The ONLY difference is the binary under test: this run uses
# the post-Phase-3 source/install_cuda/bin/sfincs (auxiliary per-step
# arrays device-canonical: the bulk zsb/zsb0 H2D in
# update_boundary_conditions is reduced to a kcs==2 sliced H2D + scatter
# kernel; the bulk uvmean/zsb/zsb0 D2H in update_boundary_fluxes is
# dropped; the kfuv D2H in compute_fluxes is dropped; the snapwave-gated
# zsm/maxzsm D2H in compute_water_levels_* moves to the dtwave-cadence
# snapwave coupling step; the zsmax D2H in compute_water_levels_* and
# update_boundary_fluxes is dropped (output cadence catches it via
# device_to_host_for_output); the per-step zsmax H2D in
# compute_water_levels_* is dropped (replaced by a device-side reset
# triggered after write_output in the USE_CUDA branch); and the
# per-step z_volume / zsderv D2H in compute_water_levels_subgrid is
# dropped. Phase 3 changes WHEN bytes move, not WHAT moves, so the
# expectation is: the auxiliary contributions to bnd_conditions_copyin,
# bnd_fluxes_copyout, mom_fluxes_copyout, and cont_subgrid_copyout drop
# to zero or near-zero, and the inner-loop steady-state per-step PCIe
# becomes dominated only by snapwave (dtwave cadence) and output
# (dtmapout cadence).
#
# Captured artifacts (committed under this directory):
#   * phase3_rank0.nsys-rep — raw NSight report (rank 0)
#   * nvtx_pushpop_sum.csv / cuda_gpu_mem_size_sum.csv — nsys stats CSVs
...
#   * SUMMARY.md            — Phase 3 delta one-pager (analyze_phase3.py)
#
# Requires: built (post-Phase-3) source/install_cuda/bin/sfincs, the
...
OUT_DIR=$REPO_ROOT/tests/perf/phase3-aux-arrays-device-canonical-20260517
RUN_DIR=$OUT_DIR/run
...
echo "=== SOR-67 Phase 3 delta capture ==="
echo "case     : $CASE_NAME"
...
CONTAINER_NAME="sfincs-phase3-$$-$(date +%s 2>/dev/null || echo 0)"
...
IN_CONTAINER_SCRIPT="$OUT_DIR/in_container_phase3.sh"
...
if [ "\$R" = "0" ]; then
  exec "$NSYS" profile \\
      ...
      --output="${OUT_CONTAINER}/phase3_rank0" "$GPU_BIN"
else
  exec "$GPU_BIN"
fi
RANK_EOF
...
ls -l "$OUT_DIR"/phase3_rank0.nsys-rep 2>/dev/null || echo "WARNING: no .nsys-rep produced"
...
docker run --rm --init \
    ...
    bash -lc '
        set -e
        NSYS=/opt/nvidia/hpc_sdk/Linux_x86_64/25.9/compilers/bin/nsys
        REP=phase3_rank0.nsys-rep
        "$NSYS" export --type sqlite --force-overwrite true \
            --output phase3_rank0.sqlite "$REP"
...
ls -l "$OUT_DIR"/phase3_rank0.sqlite "$OUT_DIR"/*.csv 2>/dev/null \
    || echo "WARNING: no sqlite / stats CSV produced"

if [ -f "$OUT_DIR/analyze_phase3.py" ]; then
    echo "--- writing SUMMARY.md ---"
    python3 "$OUT_DIR/analyze_phase3.py" "$OUT_DIR"
else
    echo "--- analyze_phase3.py absent; skipping SUMMARY.md ---"
fi

rm -rf "$RUN_DIR" "$OUT_DIR/phase3_rank0.sqlite"

echo "=== Phase 3 capture complete ==="
echo "    artifacts: $(ls "$OUT_DIR"/phase3_rank0.nsys-rep "$OUT_DIR"/SUMMARY.md 2>/dev/null | tr '\n' ' ')"
exit "$SFINCS_RC"
