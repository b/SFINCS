#!/bin/bash
# SOR-65 Phase 1 — zs device-canonical PCIe delta capture.
#
# Identical methodology to tests/perf/phase0-baseline-20260516/
# run_phase0_profile.sh (same case, same gpu_n2 config, same CUDA-aware
...
# directly comparable to the committed Phase 0 baseline. The ONLY
# difference is the binary under test: this run uses the post-Phase-1
# source/install_cuda/bin/sfincs (zs device-canonical: the five per-step
# zs H<->D bridges dropped, the kcs==6 Neumann host write ported to the
# k_update_neumann_zs device kernel). Phase 1 changes WHEN bytes move,
# not WHAT moves, so the expectation is: the zs contribution to the
# mom_fluxes_copyin / cont_*_copyin / bnd_fluxes_copyin (H2D) and the
# cont_*_copyout / bnd_fluxes_copyout (D2H) ranges drops to zero and the
# total per-step PCIe falls by ~the Phase-0 zs-attributed portion.
#
# Captured artifacts (committed under this directory):
#   * phase1_rank0.nsys-rep — raw NSight report (rank 0)
...
#   * SUMMARY.md            — Phase 1 delta one-pager (analyze_phase1.py)
#
# Requires: built (post-Phase-1) source/install_cuda/bin/sfincs, the
...
echo "=== SOR-65 Phase 1 delta capture ==="
echo "case     : $CASE_NAME"
...
CONTAINER_NAME="sfincs-phase1-$$-$(date +%s 2>/dev/null || echo 0)"
...
IN_CONTAINER_SCRIPT="$OUT_DIR/in_container_phase1.sh"
...
# in nsys (same rationale as the Phase 0 harness: profiling both ranks
# trips nsys' --kill=sigterm into mpirun's abort cascade and truncates
# rank 1's report; the 2-rank decomposition is symmetric so rank 0 is
# representative of the per-step bridge inventory).
...
if [ "\$R" = "0" ]; then
  exec "$NSYS" profile \\
      ...
      --output="${OUT_CONTAINER}/phase1_rank0" "$GPU_BIN"
else
  exec "$GPU_BIN"
fi
RANK_EOF
...
ls -l "$OUT_DIR"/phase1_rank0.nsys-rep 2>/dev/null || echo "WARNING: no .nsys-rep produced"
...
docker run --rm --init \
    ...
    bash -lc '
        set -e
        NSYS=/opt/nvidia/hpc_sdk/Linux_x86_64/25.9/compilers/bin/nsys
        REP=phase1_rank0.nsys-rep
        "$NSYS" export --type sqlite --force-overwrite true \
            --output phase1_rank0.sqlite "$REP"
...
ls -l "$OUT_DIR"/phase1_rank0.sqlite "$OUT_DIR"/*.csv 2>/dev/null \
    || echo "WARNING: no sqlite / stats CSV produced"

if [ -f "$OUT_DIR/analyze_phase1.py" ]; then
    echo "--- writing SUMMARY.md ---"
    python3 "$OUT_DIR/analyze_phase1.py" "$OUT_DIR"
else
    echo "--- analyze_phase1.py absent; skipping SUMMARY.md ---"
fi

# The .sqlite (hundreds of MB) is fully regenerable from the committed
# .nsys-rep; the staged run/ dir holds only symlinks + a copied
# sfincs.inp. Neither ships.
rm -rf "$RUN_DIR" "$OUT_DIR/phase1_rank0.sqlite"

echo "=== Phase 1 capture complete ==="
echo "    artifacts: $(ls "$OUT_DIR"/phase1_rank0.nsys-rep "$OUT_DIR"/SUMMARY.md 2>/dev/null | tr '\n' ' ')"
exit "$SFINCS_RC"
