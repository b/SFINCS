#!/bin/bash
# SOR-66 Phase 2 — q / q0 / uv / uv0 device-canonical PCIe delta capture.
#
# Identical methodology to tests/perf/phase1-zs-device-canonical-20260516/
# run_phase1_profile.sh (same case, same gpu_n2 config, same CUDA-aware
...
# directly comparable to the committed Phase 0 baseline and Phase 1
# delta. The ONLY difference is the binary under test: this run uses the
# post-Phase-2 source/install_cuda/bin/sfincs (q / q0 / uv / uv0
# device-canonical: the bridge-IN of uv at compute_fluxes gated on
# nonhydrostatic_h, the bridge-IN of q at compute_water_levels_* dropped,
# the bridge-OUTs of q / q0 / uv / uv0 at compute_fluxes and
# update_boundary_conditions dropped, the tail-only q / uv bridges in
# compute_combined_uv dropped). case_prod_compound_snapwave has
# nonhydrostatic = 0, so the gate is a no-op on this case's profile.
# Phase 2 changes WHEN bytes move, not WHAT moves, so the expectation is:
# the q / q0 / uv / uv0 contribution to mom_fluxes_copyin (gated),
# mom_fluxes_copyout, mom_combined_uv_copyout, cont_*_copyin, and
# bnd_fluxes_copyout drops to zero and the total per-step PCIe falls
# by ~the q + uv contribution measured in Phase 0 / Phase 1.
#
# Captured artifacts (committed under this directory):
#   * phase2_rank0.nsys-rep — raw NSight report (rank 0)
...
#   * SUMMARY.md            — Phase 2 delta one-pager (analyze_phase2.py)
#
# Requires: built (post-Phase-2) source/install_cuda/bin/sfincs, the
...
echo "=== SOR-66 Phase 2 delta capture ==="
echo "case     : $CASE_NAME"
...
CONTAINER_NAME="sfincs-phase2-$$-$(date +%s 2>/dev/null || echo 0)"
...
IN_CONTAINER_SCRIPT="$OUT_DIR/in_container_phase2.sh"
...
if [ "\$R" = "0" ]; then
  exec "$NSYS" profile \\
      ...
      --output="${OUT_CONTAINER}/phase2_rank0" "$GPU_BIN"
else
  exec "$GPU_BIN"
fi
RANK_EOF
...
ls -l "$OUT_DIR"/phase2_rank0.nsys-rep 2>/dev/null || echo "WARNING: no .nsys-rep produced"
...
docker run --rm --init \
    ...
    bash -lc '
        set -e
        NSYS=/opt/nvidia/hpc_sdk/Linux_x86_64/25.9/compilers/bin/nsys
        REP=phase2_rank0.nsys-rep
        "$NSYS" export --type sqlite --force-overwrite true \
            --output phase2_rank0.sqlite "$REP"
...
ls -l "$OUT_DIR"/phase2_rank0.sqlite "$OUT_DIR"/*.csv 2>/dev/null \
    || echo "WARNING: no sqlite / stats CSV produced"

if [ -f "$OUT_DIR/analyze_phase2.py" ]; then
    echo "--- writing SUMMARY.md ---"
    python3 "$OUT_DIR/analyze_phase2.py" "$OUT_DIR"
else
    echo "--- analyze_phase2.py absent; skipping SUMMARY.md ---"
fi

rm -rf "$RUN_DIR" "$OUT_DIR/phase2_rank0.sqlite"

echo "=== Phase 2 capture complete ==="
echo "    artifacts: $(ls "$OUT_DIR"/phase2_rank0.nsys-rep "$OUT_DIR"/SUMMARY.md 2>/dev/null | tr '\n' ' ')"
exit "$SFINCS_RC"
