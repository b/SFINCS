...
# capture.
#
# Identical methodology to tests/perf/phase3-aux-arrays-device-canonical-20260517/
# run_phase3_profile.sh (same case, same gpu_n2 config, same CUDA-aware
...
# Phase 3 captures. The ONLY difference is the binary under test: this
# run uses the post-Phase-4 source/install_cuda/bin/sfincs (new
# flush_device_to_host_state / pull_host_state_to_device helpers wired
# around the wavemaker / nonhydrostatic / BMI call sites).
#
# case_prod_compound_snapwave has all three features OFF (no wavemaker,
# no nonhydrostatic, no BMI), so the helper predicates `if (wavemaker)`,
# `if (nonhydrostatic)`, and the BMI entry points are never entered.
# The expectation is: the inner-loop steady-state per-step PCIe is
# byte-for-byte identical to the Phase 3 baseline -- Phase 4 changes
# the code path on feature-ON cases only.
#
# Captured artifacts (committed under this directory):
#   * phase4_rank0.nsys-rep -- raw NSight report (rank 0)
...
#   * SUMMARY.md            -- Phase 4 delta one-pager (analyze_phase4.py)
#
...
OUT_DIR=$REPO_ROOT/tests/perf/phase4-bridge-helpers-20260517
RUN_DIR=$OUT_DIR/run
...
echo "=== SOR-68 Phase 4 delta capture ==="
echo "case     : $CASE_NAME"
...
CONTAINER_NAME="sfincs-phase4-$$-$(date +%s 2>/dev/null || echo 0)"
...
IN_CONTAINER_SCRIPT="$OUT_DIR/in_container_phase4.sh"
...
if [ "\$R" = "0" ]; then
  exec "$NSYS" profile \\
      ...
      --output="${OUT_CONTAINER}/phase4_rank0" "$GPU_BIN"
else
  exec "$GPU_BIN"
fi
RANK_EOF
...
ls -l "$OUT_DIR"/phase4_rank0.nsys-rep 2>/dev/null || echo "WARNING: no .nsys-rep produced"
...
docker run --rm --init \
    ...
    bash -lc '
        set -e
        NSYS=/opt/nvidia/hpc_sdk/Linux_x86_64/25.9/compilers/bin/nsys
        REP=phase4_rank0.nsys-rep
        "$NSYS" export --type sqlite --force-overwrite true \
            --output phase4_rank0.sqlite "$REP"
...
ls -l "$OUT_DIR"/phase4_rank0.sqlite "$OUT_DIR"/*.csv 2>/dev/null \
    || echo "WARNING: no sqlite / stats CSV produced"

if [ -f "$OUT_DIR/analyze_phase4.py" ]; then
    echo "--- writing SUMMARY.md ---"
    python3 "$OUT_DIR/analyze_phase4.py" "$OUT_DIR"
else
    echo "--- analyze_phase4.py absent; skipping SUMMARY.md ---"
fi

rm -rf "$RUN_DIR" "$OUT_DIR/phase4_rank0.sqlite"

echo "=== Phase 4 capture complete ==="
echo "    artifacts: $(ls "$OUT_DIR"/phase4_rank0.nsys-rep "$OUT_DIR"/SUMMARY.md 2>/dev/null | tr '\n' ' ')"
exit "$SFINCS_RC"
