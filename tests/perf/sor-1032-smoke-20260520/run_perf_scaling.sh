...
#   * grid      — 1x always; 4x for every production case. Four of
#                 the five cases ship a committed
#                 ``tests/cases/<case>/4x/`` subdir whose contents
#                 are staged for the 4x cell. The fifth
#                 (``case_prod_regular_tide``) is refined at sweep
#                 time by ``generate_4x_regular_tide.py``.
...
stage_case() {
    # Stage a case into ``run_dir`` for the requested ``grid``.
    #   * ``grid=1x`` — stage from ``tests/cases/<case>/``.
    #   * ``grid=4x`` — stage from ``tests/cases/<case>/4x/`` when it
    #     exists (a committed pre-refined input set); otherwise fall
    #     back to ``tests/cases/<case>/`` and rely on the caller's
    #     runtime-refinement hook (currently
    #     ``refine_regular_tide_4x`` for ``case_prod_regular_tide``).
    local case_dir=$1 run_dir=$2 grid=$3
    local src_dir=$case_dir
    if [ "$grid" = "4x" ] && [ -d "$case_dir/4x" ]; then
        src_dir=$case_dir/4x
    fi
    mkdir -p "$run_dir"
    ...
    for entry in "$src_dir"/*; do
        ...
        base=$(basename "$entry")
        case "$base" in
            README.md|fetch.sh|generate.py|4x) ;;
            generate_4x_*.py) ;;
...
run_cpu_cell() {
    ...
    stage_case "$case_dir" "$run_dir" "$grid"
    if [ "$grid" = "4x" ] && [ ! -d "$case_dir/4x" ]; then
        refine_regular_tide_4x "$run_dir"
    fi
