#!/bin/bash
# SOR-69 Phase 5 — final canonical-on-device NSight profile capture.
#
# Identical methodology to tests/perf/phase4-bridge-helpers-20260517/
# run_phase4_profile.sh and the Phase 0 NVTX-annotated baseline in
# tests/perf/phase0-baseline-20260516/run_phase0_profile.sh: same case
# (case_prod_compound_snapwave), same gpu_n2 config, same CUDA-aware
# HPC-X 2.24 container, same 60 s steady-state NSight window after a
# 20 s startup-skip delay. The ONLY differences are (a) the binary
# under test is the post-Phase-1+2+3+4 cumulative-stack build of
# source/install_cuda/bin/sfincs and (b) the captured artifacts are
# committed under tests/perf/phase5-canonical-on-device-final-20260517/
# rather than under a per-phase delta directory.
#
# This is the AC #3 driver for SOR-69 Phase 5: the captured profile
# is the empirical proof that the canonical-on-device refactor (a)
# pushed GPU SM% meaningfully higher than the Phase-0 baseline, (b)
# dropped PCIe steady-state Tx/Rx in the inner loop to near zero with
# spikes only at the operator-set dtwave=1800 s and dtmapout intervals,
# (c) dropped rank-0 CPU% meaningfully below the Phase-0 100 % pegged
# measurement, and (d) shortened wall clock for the case. The four
# metrics are reported as measured numbers in SUMMARY.md (the AC notes
# they are environment-dependent and not gated mechanically).
#
...
#   * SUMMARY.md            -- Phase 5 vs Phase 0 one-pager (human-authored)
#
# Requires: built (post-Phase-4) source/install_cuda/bin/sfincs, the
...
echo "=== SOR-69 Phase 5 final capture ==="
echo "case     : $CASE_NAME"
...
CONTAINER_NAME="sfincs-phase5-$$-$(date +%s 2>/dev/null || echo 0)"
...
IN_CONTAINER_SCRIPT="$OUT_DIR/in_container_phase5.sh"
...
if [ "\$R" = "0" ]; then
  exec "$NSYS" profile \\
      ...
      --output="${OUT_CONTAINER}/phase5_rank0" "$GPU_BIN"
else
  exec "$GPU_BIN"
fi
RANK_EOF
...
ls -l "$OUT_DIR"/phase5_rank0.nsys-rep 2>/dev/null || echo "WARNING: no .nsys-rep produced"
...
docker run --rm --init \
    ...
    bash -lc '
        set -e
        NSYS=/opt/nvidia/hpc_sdk/Linux_x86_64/25.9/compilers/bin/nsys
        REP=phase5_rank0.nsys-rep
        "$NSYS" export --type sqlite --force-overwrite true \
            --output phase5_rank0.sqlite "$REP"
...
ls -l "$OUT_DIR"/phase5_rank0.sqlite "$OUT_DIR"/*.csv 2>/dev/null \
    || echo "WARNING: no sqlite / stats CSV produced"

rm -rf "$RUN_DIR" "$OUT_DIR/phase5_rank0.sqlite"

echo "=== Phase 5 capture complete ==="
echo "    artifacts: $(ls "$OUT_DIR"/phase5_rank0.nsys-rep 2>/dev/null | tr '\n' ' ')"
exit "$SFINCS_RC"
