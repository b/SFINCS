#!/bin/bash
# SOR-81 SnapWave characterization profile capture.
#
...
#
# Same case as SOR-69 Phase 5 (case_prod_compound_snapwave, gpu_n2) and
# same CUDA-aware HPC-X 2.24 container, so the captures stack with the
# Phase 5 PCIe / SM% numbers in tests/perf/phase5-canonical-on-device-
# final-20260517/SUMMARY.md.
#
...
#   * snapwave_perf_report.txt  -- perf report --stdio --sort=overhead
#   * sfincs.log                -- SFINCS log (timing summary, SnapWave per-call)
#   * sfincs.stdout             -- mpirun + nsys/perf stdout
#
# Requires: built source/install_cuda/bin/sfincs (NVTX SnapWave range
# from this issue), the sfincs-build-gpu:latest image, and 2 GPUs.
...
OUT_DIR=$REPO_ROOT/tests/perf/snapwave-characterization-20260517
