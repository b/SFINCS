# case_prod_quadtree_subgrid_tide — 4x grid refinement

A 4x-cell-count refinement of the parent
[`case_prod_quadtree_subgrid_tide`](../README.md) case. The 1x quadtree
mesh is built on a 130 × 130 base grid at `dx = dy = 200` m; this 4x
variant doubles the base grid to 260 × 260 and halves the base `dx`
to 100 m, keeping the physical footprint (26 km × 26 km) and the
...
(≈ 3.94×), and the subgrid lookup carries one entry per active cell.
...
(`build_refinement_polygons`), boundary forcing (M2 + M4 + 0.20 m
sea-level on the western open boundary), and time window (24 h at
30 min boundary sampling) are unchanged — only the base grid
resolution differs, isolating resolution as the independent axis along
which the perf-scaling sweep traverses the case.
...
mesh build (`build_quadtree_xugrid`, `cut_inactive_cells`) and the
subgrid table build (`build_subgrid_table_quadtree`), so unlike the
analytical-numpy cases (riverine, storm_amuv) the pinned HydroMT-SFINCS
package IS exercised at refinement time. Bumping the pin requires
re-cutting this directory; see
...
- **Bathymetry**: `synth_bathymetry(x, y, mmax_m)` — a linear
  east → west sea → shore gradient (−10 m offshore at the western
...
- **Refinement geometry**: `build_refinement_polygons` — two concentric
  ...
  imported shapefile.
- **Mask layout**: western 2 fine-pixel columns are open boundary
  (`mask = 2`), interior is active (`mask = 1`), exterior outside the
  rectangular footprint is inactive (`mask = 0`).
- **Boundary forcing**: M2 (12.4206 h, 0.60 m amplitude) + M4
  (6.2103 h, 0.15 m amplitude with +0.6 rad phase) + 0.20 m sea-level
  constant, sampled every 30 min for 24 h, applied identically at two
  `bndfile` points on the western edge.
- **Grid constants** passed by `generate_4x_quadtree_subgrid_tide.py`:
  - `--nmax 260` (was 130)
  - `--mmax 260` (was 130)
  - `--dx 100.0` (was 200.0)
...
(default: `tests/cases/case_prod_quadtree_subgrid_tide/4x/`):
...
| File | Description |
| --- | --- |
| `sfincs.inp` | Run configuration — copied verbatim from the 1x parent case. No `mmax` / `nmax` / `dx` / `dy` lines (the quadtree mesh is loaded from `qtrfile = sfincs.nc`); `tref` / `tstart` / `tstop` / `qtrfile` / `sbgfile` / `bndfile` / `bzsfile` and subgrid-relevant flags (`huthresh`, `inputformat`, `outputformat`, `bndtype`, `epsg`, `crsgeo`) all match 1x. |
...
| `sfincs.bnd` | Two boundary support points at the centre of the leftmost base column, `y ∈ {0.25, 0.75} × 26 000` m. |
