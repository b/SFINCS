...
* The SFINCS GPU build (CUDA Fortran, ``--enable-cuda``) is functional and validated; see :doc:`gpu` for build / run / tuning / troubleshooting. The legacy OpenACC GPU container path is no longer maintained.
