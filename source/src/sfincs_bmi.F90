...
#ifdef USE_CUDA
   ! BMI getters and setters exchange data with the host shadows of
   ! catalog state arrays (zs, q, uv, zsmax, qext). The inner per-step
   ! loop is device-canonical post-SOR-65 / SOR-66 / SOR-67, so the
   ! host shadows otherwise stay stale across each sfincs_update.
   ! Refresh host before returning a pointer (get_value_ptr) and pull
   ! caller writes back to device before the next sfincs_update at the
   ! update / update_until entry (SOR-68 Phase 4). The CPU build has
   ! no device shadows; these helpers are not compiled in for it.
   use sfincs_partition, only: flush_device_to_host_state, &
                               pull_host_state_to_device, &
                               bundle_for_bmi
#endif
   !
   implicit none
...
#ifdef USE_CUDA
      ! Caller may have written to host shadows via a prior
      ! get_value_ptr; pull those edits to device before sfincs_update
      ! runs the device-canonical inner loop. Flush at exit so the
      ! next get_value_ptr hands back a current host view.
      call pull_host_state_to_device(bundle_for_bmi)
#endif
      unused_t = -1.0
      ierr = sfincs_update(unused_t)
#ifdef USE_CUDA
      call flush_device_to_host_state(bundle_for_bmi)
#endif
...
#ifdef USE_CUDA
      ! Refresh host shadows before handing the caller a pointer into
      ! them, so subsequent caller reads see the device-canonical
      ! values from the most recent sfincs_update.
      call flush_device_to_host_state(bundle_for_bmi)
#endif
